...
export function sendReturnNudgeEmail({ toEmail, firstName }) {
  return sendEmail({
    to: toEmail,
    subject: `${firstName ? firstName + ', ' : ''}Mahoro's still here whenever you're ready 🌿`,
    html: baseWrapper(`
      <h1 style="font-size:22px;">No pressure${firstName ? `, ${firstName}` : ''} — just a small hello 🌿</h1>
      <p>You talked with Mahoro a few days ago. There's no obligation to come back on any schedule — but if something's on your mind today, Mahoro's right there whenever you want to pick it back up.</p>
      <a href="https://amahoro.app" style="display:inline-block;margin-top:16px;padding:12px 20px;background:#25D366;color:#000;text-decoration:none;border-radius:6px;font-weight:700;">Talk to Mahoro</a>
    `),
  });
}
