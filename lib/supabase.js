...
/** Call a Postgres function (RPC), e.g. sbRpc('get_users_needing_nudge') */
export async function sbRpc(functionName, args = {}) {
  const res = await fetch(`${BASE}/rpc/${functionName}`, {
    method: 'POST',
    headers: headers(),
    body: JSON.stringify(args),
  });
  if (!res.ok) throw new Error(`Supabase RPC ${functionName} failed: ${res.status} ${await res.text()}`);
  return res.json();
}
