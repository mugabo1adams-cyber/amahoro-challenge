          ...
          {[
            ...
            <div key={p.name} style={{ background: COLORS.card, border: `2px solid ${p.featured ? COLORS.green : COLORS.border}`, borderRadius: 16, padding: "28px 24px", position: "relative", boxShadow: p.featured ? `0 0 0 1px rgba(37,211,102,0.2), 0 20px 40px rgba(37,211,102,0.07)` : "none" }}>
              ...
              <button onClick={() => {
                ...
                if (p.btnStyle === "sky") { window.location.href = "mailto:hello@amahoro.app?subject=Organization Plan Inquiry"; return; }
                // Paystack payment
                const handler = window.PaystackPop.setup({
                  key: import.meta.env.VITE_PAYSTACK_PUBLIC_KEY,
                  email: prompt("Enter your email to continue:") || "user@amahoro.app",
                  amount: p.name === "Amahoro Pro" ? 400 * 100 : 2900 * 100,
                  currency: "KES",
                  ref: "amahoro_" + Math.floor(Math.random() * 1000000000),
                  metadata: { plan: p.name, product: "Amahoro Challenge" },
                  callback: function(response) {
                    alert("✅ Payment successful! Reference: " + response.reference + "\n\nYour Pro account is now active. Welcome to Amahoro Pro! 🌿");
                  },
  ...
  return (
    <div style={{ minHeight: "100vh", background: COLORS.night, display: "flex", flexDirection: "column", fontFamily: "DM Sans,sans-serif", color: COLORS.text }}>
      ...
      <div style={{ background: COLORS.deep, borderTop: `1px solid ${COLORS.border}`, padding: "14px 4%", display: "flex", gap: 10, alignItems: "center" }}>
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} placeholder="Talk to Amara — English, Français, Kiswahili, Kinyarwanda 🌿" style={{ flex: 1, background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: "12px 16px", color: COLORS.text, fontFamily: "DM Sans,sans-serif", fontSize: "0.92rem", transition: "border-color 0.2s" }} />
