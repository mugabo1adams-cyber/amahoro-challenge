...
const CHALLENGES = [
  { day:1, theme:"Awareness", emoji:"🧠", duration:"5 min", title:"Name what you feel.", context:"In our culture, we are taught to stay strong and not talk about our struggles. But when you name what you feel, it becomes smaller. This is your first brave step.", task:'Finish this sentence three times: "Right now I feel worried about _____ because _____. My body feels _____ when this happens." Then answer one more question: "What do I — or my body — need right now to feel a little better today?"', placeholder:"Right now I feel worried about my exams because my family expects me to be the first graduate. My body feels tight in my chest when this happens... What I need right now is a few minutes to breathe and remember I am doing my best.", tip:"Naming what you feel is not weakness. It is the first step." },
...
const MAHORO_SYSTEM = `You are Mahoro, a warm wellness companion for African youth from Amahoro. You are NOT a therapist, doctor, lawyer, or expert in any field. You are a caring friend — someone who listens deeply, understands African realities, and walks alongside people as they find their own strength.
    ...
    <div style={{ minHeight:"100vh", background:C.night, display:"flex", alignItems:"center", justifyContent:"center", padding:"2rem", fontFamily:"DM Sans,sans-serif" }}>
      ...
      <div className="fade-in" style={{ width:"100%", maxWidth:420 }}>
        <div style={{ textAlign:"center", marginBottom:32 }}>
          ...
          <div style={{ color:C.muted, fontSize:"0.9rem" }}>Your growth companion. Here whenever you need it.</div>
        </div>
...
function AdminDashboard({ onBack }) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // now via a locked-down admin-only function instead of an exposed view
      const { data: authData, error: authError } = await supabase.rpc("get_admin_signups");
      if (authError) console.error("get_admin_signups RPC failed:", authError);
      ...
      // Use auth signups as the true total — but if that RPC call failed for any
      // reason (see console for authError above), fall back to counting completed
      // profiles instead of showing a false zero. This undercounts people who
      // signed up but never finished onboarding, but it's honest, not broken.
      const signupSource = (authData && authData.length > 0) ? authData : profileData;
      const totalUsers = signupSource?.length || 0;
      const completedOnboarding = profileData?.length || 0;
      const newThisWeek = signupSource?.filter(u => new Date(u.created_at) > weekAgo).length || 0;
      const newThisMonth = signupSource?.filter(u => new Date(u.created_at) > monthAgo).length || 0;
      ...
      <div style={{ maxWidth:1000, margin:"0 auto", padding:"2rem 4%" }}>
        ...
        {tab==="overview" && stats && (
          <div>
            ...
            <p style={{ color:C.muted, fontSize:"0.85rem", marginBottom:24 }}>Last updated just now · {stats.totalUsers} users · {stats.countries.length} countries</p>
        ...
        {tab==="users" && (
          <div>
            <h2 style={{ fontFamily:"Sora,sans-serif", fontWeight:800, fontSize:"1.5rem", marginBottom:24 }}>All Users ({profiles.length})</h2>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, overflow:"hidden", marginBottom:16 }}>
              {profiles.slice((page-1)*PAGE_SIZE, page*PAGE_SIZE).map((u, i, arr) => (
              ...
              {profiles.length===0 && <div style={{ padding:"30px 18px", textAlign:"center", color:C.muted, fontSize:"0.88rem" }}>No users yet.</div>}
            </div>
            {profiles.length > PAGE_SIZE && (
              <div style={{ display:"flex", justifyContent:"center", alignItems:"center", gap:14 }}>
                ...
                <span style={{ color:C.muted, fontSize:"0.85rem", fontFamily:"Sora,sans-serif" }}>Page {page} of {Math.ceil(profiles.length/PAGE_SIZE)}</span>
                <button onClick={() => setPage(p => Math.min(Math.ceil(profiles.length/PAGE_SIZE), p+1))} disabled={page>=Math.ceil(profiles.length/PAGE_SIZE)}
                  style={{ background:"transparent", border:`1px solid ${C.border}`, color:page>=Math.ceil(profiles.length/PAGE_SIZE)?C.faint:C.text, borderRadius:8, padding:"7px 16px", cursor:page>=Math.ceil(profiles.length/PAGE_SIZE)?"not-allowed":"pointer", fontFamily:"Sora,sans-serif", fontSize:"0.85rem" }}>Next →</button>
              </div>
            )}
          </div>
        )}
          ...
          <div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))", gap:10 }}>
              {stats.countries.map(country => {
                const count = profiles.filter(u => u.country === country).length;
                return (
...
// ─── PRO SCREEN ───────────────────────────────────────────────
function ProScreen({ user, profile, isPro, isOrg, onActivatePro, onActivateOrg, onBack, onLegal }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (org) {
        ...
        const { data: stats } = await supabase.rpc("get_org_engagement_stats", { org_id: org.id });
        if (stats && stats.length > 0) setOrgStats(stats[0]);
      }
    })();
  }, [isOrg, user]);
  ...
  const generate = async (feat) => {
    setLoading(true); setOutput("");
    try {
      let prompt = feat.prompt;

      // For progress reports specifically: pull the most recent previous one
      // and give Mahoro that context, so it can genuinely reflect change
      // over time instead of writing an isolated snapshot each time.
      if (feat.key === "progress") {
        ...
        if (prev && prev.length > 0) {
          prompt += `\n\nHere is their previous progress report, from ${new Date(prev[0].created_at).toLocaleDateString()}: "${prev[0].content}". In this new report, genuinely reflect what has changed since then — new focus, new goals, real progress — rather than repeating the same points.`;
        }
      }

      const result = await callClaude(MAHORO_SYSTEM, prompt, user?.id);
      setOutput(result);

      // Auto-save so nothing is lost if the person navigates away without
      // copying it. Org-tool generations (workshop/groupreport/onboarding)
      // are tagged with the organization; personal tools are not.
      const orgToolKeys = ["workshop", "groupreport", "onboarding"];
      await supabase.from("generated_content").insert({
        user_id: user.id,
        organization_id: orgToolKeys.includes(feat.key) ? organizationId : null,
        type: feat.key,
        label: feat.label,
        content: result
      });
  ...
  const orgFeatures = [
    ...
    { key:"groupreport", label:"📈 Group Progress Report", desc:"A professional report for your organization.", prompt:`Write a simple one-page group wellness progress report for ${orgName||"an organization"} using Amahoro. Include: program overview, key benefits, what participants are learning, recommendations for next month, closing note for leadership. Simple English. Make it positive and practical.` },
    { key:"onboarding", label:"📋 Staff Guide", desc:"A simple guide for your staff.", prompt:`Write a simple staff guide for facilitators introducing Amahoro to young people at ${orgName||"an organization"}. Include: how to introduce the program, how to support participants, common questions and answers, how to help someone struggling, how to celebrate completions. Very simple English. Warm and practical.` },
  ];
        ...
        {!isPro && (
          <div className="fade-in">
            ...
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"22px", marginBottom:16 }}>
              ...
              <div style={{ marginTop:6, fontSize:"0.72rem", color:C.muted, textAlign:"center" }}>
                By subscribing, you agree to our <button onClick={() => onLegal("terms")} style={{ background:"none", border:"none", color:C.green, cursor:"pointer", fontSize:"0.72rem", padding:0, textDecoration:"underline" }}>Terms</button> and <button onClick={() => onLegal("refund")} style={{ background:"none", border:"none", color:C.green, cursor:"pointer", fontSize:"0.72rem", padding:0, textDecoration:"underline" }}>Refund Policy</button>
              </div>
            </div>
            ...
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"22px" }}>
              ...
              <div style={{ marginTop:6, fontSize:"0.72rem", color:C.muted, textAlign:"center" }}>
                By subscribing, you agree to our <button onClick={() => onLegal("terms")} style={{ background:"none", border:"none", color:"#4FC3F7", cursor:"pointer", fontSize:"0.72rem", padding:0, textDecoration:"underline" }}>Terms</button> and <button onClick={() => onLegal("refund")} style={{ background:"none", border:"none", color:"#4FC3F7", cursor:"pointer", fontSize:"0.72rem", padding:0, textDecoration:"underline" }}>Refund Policy</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="fade-in">
              ...
              <>
                ...
                {orgStats && (
                  <div style={{ marginBottom:24 }}>
                    <h3 style={{ fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.85rem", color:"#4FC3F7", textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:12 }}>Cohort Engagement</h3>
                    <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:10 }}>
                      {[["Members", orgStats.total_members], ["Active This Week", orgStats.active_this_week], ["3+ Days Done", orgStats.completed_3plus_days]].map(([label, val]) => (
                        <div key={label} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"16px" }}>
                          <div style={{ fontFamily:"Sora,sans-serif", fontWeight:800, fontSize:"1.6rem", color:"#4FC3F7" }}>{val}</div>
                          <div style={{ fontSize:"0.78rem", color:C.muted, marginTop:2 }}>{label}</div>
                        </div>
                      ))}
                    </div>
                    <div style={{ fontSize:"0.72rem", color:C.muted, marginTop:8 }}>Counts only — individual conversations always stay private, even from you.</div>
                  </div>
                )}
            ...
            {activeFeature && (
              <div className="fade-in" style={{ background:C.deep, border:`1px solid ${C.border}`, borderRadius:14, padding:"20px" }}>
                ...
                <button onClick={() => generate(activeFeature)} style={{ background:C.green, border:"none", color:"#000", fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.9rem", padding:"10px 22px", borderRadius:8, cursor:"pointer", marginBottom:loading||output?16:0 }}>
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                      ...
                      <div style={{ display:"flex", gap:8 }}>
                        ...
                        <button onClick={() => {
                          ...
                          a.href = url; a.download = `${activeFeature.label.replace(/[^\w\s]/g,"").trim()}.txt`;
                          a.click(); URL.revokeObjectURL(url);
                        }} style={{ background:"transparent", border:`1px solid ${C.border}`, color:C.muted, fontFamily:"Sora,sans-serif", fontSize:"0.75rem", padding:"4px 12px", borderRadius:6, cursor:"pointer" }}>Download</button>
                      </div>
                    </div>
                    <div style={{ fontSize:"0.9rem", lineHeight:1.75, color:C.text, whiteSpace:"pre-wrap" }}>{output}</div>
                    <div style={{ marginTop:12, fontSize:"0.75rem", color:C.muted }}>✓ Saved automatically — find it anytime in your Progress tab. Copy or download it too if you'd like your own file.</div>
                  </div>
                )}
              </div>
            )}
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.night, display:"flex", alignItems:"center", justifyContent:"center", padding:"2rem", fontFamily:"DM Sans,sans-serif" }}>
      ...
      <div style={{ width:"100%", maxWidth:520 }}>
        <div style={{ fontFamily:"Sora,sans-serif", fontWeight:800, fontSize:"1.1rem", color:C.green, marginBottom:40, textAlign:"center" }}>🌿 Amahoro</div>
...
function ChallengeScreen({ user, profile, onCoach, onHome, isPro, isOrg, onUpgrade, onSignup }) {
  ...
  // Load the person's saved workbook/meditation/progress-report/new-challenge
  // history when they open the Progress tab — this is the fix for "I
  // generated something and it just disappeared."
  useEffect(() => {
    if (view !== "progress" || !user?.id) return;
    (async () => {
      const { data } = await supabase
        .from("generated_content")
        .select("id, type, label, content, created_at")
        .eq("user_id", user.id)
  ...
  // Load saved progress from Supabase, then land the person on the day
  // they're actually up to — not always Day 1. Someone on Day 5 shouldn't
  // have to click through Days 1-4 every time they come back.
  useEffect(() => {
    const loadProgress = async () => {
      if (!user?.id) return;
      const { data } = await supabase.from("progress").select("day_number").eq("user_id", user.id);
      if (data && data.length > 0) {
        const completed = data.map(p => p.day_number - 1);
        setCompletedDays(completed);
        const highestCompleted = Math.max(...completed);
        const nextDay = Math.min(highestCompleted + 1, 20); // cap at Day 21 (index 20)
  ...
  const completeDay = useCallback(async () => {
    ...
    try {
      const tip = await callClaude(MAHORO_SYSTEM, `My name is ${profile?.name}, from ${profile?.country}. My main worry is: ${profile?.trigger_type}. Today is Day ${ch.day} of the Amahoro 21-Day Path. Theme: "${ch.title}". The task I was asked to do today: ${ch.task}. My reflection: "${reflection}". Give me your coaching response. Very simple English — short sentences, easy words. Be warm and specific to what I shared. If my reflection only covers part of what today's task asked, gently and warmly invite me to share the rest too — never in a way that makes me feel I did something wrong, just a soft "and what about..." kind of nudge.`, user?.id);
      setAiTip(tip);
      ...
      // Also save this exchange into the shared conversation history, so if
      // the person continues into the full chat, Mahoro's question here
      // isn't forgotten — it's the same ongoing conversation, not a dead end.
      if (user?.id) {
        await supabase.from("messages").insert([
          { user_id: user.id, role: "user", content: `Day ${ch.day} — ${ch.title}: ${reflection}` },
          { user_id: user.id, role: "assistant", content: tip }
        ]);
      }
            ...
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, overflow:"hidden" }}>
              ...
              <div style={{ padding:"24px" }}>
                ...
                <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                  ...
                  <a href={`https://wa.me/?text=${encodeURIComponent("I am on the Amahoro 21-Day Path — it is free and genuinely helpful. Try it: amahoro.app 🌿")}`} target="_blank" rel="noreferrer" style={{ display:"flex", alignItems:"center", gap:6, background:"transparent", border:`1px solid ${C.border}`, color:C.muted, fontFamily:"Sora,sans-serif", fontWeight:600, fontSize:"0.9rem", padding:"11px 18px", borderRadius:8, textDecoration:"none" }}>🌿 Share Amahoro</a>
                  ...
                  <div ref={tipRef} className="fade-in" style={{ background:"rgba(37,211,102,0.05)", border:"1px solid rgba(37,211,102,0.2)", borderRadius:12, padding:"18px", marginTop:20 }}>
                    ...
                    {tipTyping?<TypingDots/>:(
                      <>
                        ...
                        {user && (
                          <button onClick={onCoach} style={{ marginTop:14, display:"flex", alignItems:"center", gap:6, background:"transparent", border:`1px solid ${C.green}`, color:C.green, fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.85rem", padding:"9px 18px", borderRadius:8, cursor:"pointer" }}>
                            💬 Reply to Mahoro →
                          </button>
                        )}
                        {!user && currentDay === 0 && (
          ...
          <div className="fade-in">
            ...
            {isPro && (
              <div style={{ marginTop:32 }}>
                <h3 style={{ fontFamily:"Sora,sans-serif", fontWeight:800, fontSize:"1.15rem", marginBottom:4 }}>My Library</h3>
                <p style={{ color:C.muted, fontSize:"0.85rem", marginBottom:16 }}>Every workbook, meditation, and report you've generated — saved automatically, always here.</p>
                {library.length === 0 ? (
                  <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"20px", color:C.muted, fontSize:"0.85rem", textAlign:"center" }}>Nothing generated yet — try a Pro tool and it'll show up here.</div>
                ) : (
                  <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                    {library.map(item => (
                      <div key={item.id} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, overflow:"hidden" }}>
                        <div onClick={() => setLibraryOpen(libraryOpen===item.id?null:item.id)} style={{ padding:"14px 18px", cursor:"pointer", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                          <div>
                            <div style={{ fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.9rem" }}>{item.label}</div>
                            <div style={{ fontSize:"0.75rem", color:C.muted, marginTop:2 }}>{new Date(item.created_at).toLocaleDateString(undefined,{month:"short",day:"numeric",year:"numeric"})}</div>
                          </div>
                          <span style={{ color:C.muted, fontSize:"0.8rem" }}>{libraryOpen===item.id?"▲":"▼"}</span>
                        </div>
                        {libraryOpen===item.id && (
                          <div style={{ padding:"0 18px 18px" }}>
                            <div style={{ background:C.deep, borderRadius:8, padding:"14px", fontSize:"0.88rem", lineHeight:1.7, color:C.text, whiteSpace:"pre-wrap", marginBottom:10 }}>{item.content}</div>
                            <div style={{ display:"flex", gap:8 }}>
                              <button onClick={() => navigator.clipboard.writeText(item.content).then(() => alert("Copied!"))} style={{ background:"transparent", border:`1px solid ${C.border}`, color:C.muted, fontFamily:"Sora,sans-serif", fontSize:"0.75rem", padding:"5px 14px", borderRadius:6, cursor:"pointer" }}>Copy</button>
                              <button onClick={() => {
                                const blob = new Blob([item.content], { type: "text/plain" });
                                const url = URL.createObjectURL(blob);
                                const a = document.createElement("a");
                                a.href = url; a.download = `${item.label.replace(/[^\w\s]/g,"").trim()}.txt`;
                                a.click(); URL.revokeObjectURL(url);
                              }} style={{ background:"transparent", border:`1px solid ${C.border}`, color:C.muted, fontFamily:"Sora,sans-serif", fontSize:"0.75rem", padding:"5px 14px", borderRadius:6, cursor:"pointer" }}>Download</button>
                            </div>
                          </div>
                        )}
                      </div>
...
function CoachScreen({ user, profile, onBack, isPro, isOrg }) {
  ...
  const toggleListening = () => {
    ...
    recognition.lang = "en-US"; // browser auto-detects reasonably well across supported languages regardless
    let finalTranscript = "";
  ...
  const send = useCallback(async () => {
    ...
    setLoading(false);
  }, [input, messages, loading, profile, user, listening]);
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.night, display:"flex", flexDirection:"column", fontFamily:"DM Sans,sans-serif", color:C.text }}>
      ...
      <div style={{ background:C.deep, borderTop:`1px solid ${C.border}`, padding:"14px 4%", display:"flex", gap:10, alignItems:"center" }}>
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key==="Enter"&&send()} placeholder={listening ? "Listening... speak now 🎙️" : "Talk to Mahoro — English, Français, Kiswahili, Kinyarwanda 🌿"} style={{ flex:1, background:C.card, border:`1px solid ${listening?C.green:C.border}`, borderRadius:10, padding:"12px 16px", color:C.text, fontFamily:"DM Sans,sans-serif", fontSize:"0.92rem", transition:"border-color 0.2s" }} />
        {voiceSupported && (
          <button onClick={toggleListening} title={listening ? "Stop recording" : "Speak instead of typing"} style={{
            background: listening ? "#ef4444" : C.card, border:`1px solid ${listening?"#ef4444":C.border}`,
            color: listening ? "#fff" : C.muted, width:44, height:44, borderRadius:10, cursor:"pointer",
...
These Terms of Service ("Terms") are a legal agreement between you and Amahoro ("we," "us"). Amahoro is a product of KUZOO.AI Ltd, a company limited by shares registered in Rwanda (TIN 156296596), based in Kigali, Rwanda. They govern your use of amahoro.app and related services (the "Service"). By creating an account or using the Service, you agree to these Terms.

2. WHO CAN USE AMAHORO
You must be at least 16 years old to use the Service. If you are between 16 and 18, you should have a parent or guardian's permission. The Service is not intended for children under 16.

3. WHAT AMAHORO IS — AND ISN'T
Amahoro offers a 21-day self-guided wellness and growth challenge, and an AI companion ("Mahoro") for supportive conversation in English, French, Swahili, and Kinyarwanda. Mahoro is an AI system, not a licensed therapist, counselor, doctor, or mental health professional. Nothing Mahoro says is medical, psychological, legal, or financial advice. If you're in crisis or facing a medical emergency, please contact local emergency services or a crisis helpline. Mahoro's responses are generated using third-party AI technology (currently Anthropic's Claude) and may occasionally be inaccurate.

4. YOUR ACCOUNT
Keep your login credentials confidential. Provide accurate information. We may suspend or terminate accounts that violate these Terms.

5. SUBSCRIPTIONS AND BILLING
Free: the core 21-Day Path and limited Mahoro access, no cost. Pro — $4/month: unlimited coaching, workbook, meditation content, progress reports, new challenges. Organizations — $150/month or $1,500/year: everything in Pro, for up to 30 members, plus workshop plans, group reports, and a staff guide. Prices are billed in Kenyan Shillings (KES) via Paystack — you can pay with any major card or local payment method. Subscriptions auto-renew until cancelled. Cancel anytime in account settings or by emailing hello@amahoro.app.

6. ORGANIZATIONS PLAN
The Organization Owner can invite up to 30 members and is responsible for who has access to the organization's workspace.

7. ACCEPTABLE USE
Don't use the Service unlawfully or to harm others; don't try to gain unauthorized access to accounts or systems that aren't yours; don't harass other users or misuse Mahoro to generate harmful content; don't scrape or abuse the Service; don't misuse the refund process.

8. YOUR CONTENT
You own what you share with Mahoro. We use it only to run and improve the Service, per our Privacy Policy. We don't sell your content.

9. INTELLECTUAL PROPERTY
The Service — its design, branding, and content — belongs to us or our licensors.

10. DISCLAIMERS AND LIMITATION OF LIABILITY
The Service is provided "as is." Our total liability for any claim is limited to the amount you paid us in the 12 months before the claim.

11. ENDING YOUR USE OF THE SERVICE
Delete your account anytime by emailing hello@amahoro.app.

12. GOVERNING LAW
These Terms are governed by the laws of Rwanda.

13. CHANGES TO THESE TERMS
We'll notify active users of material changes via email or in-app notice.
...
1. WHO WE ARE
Amahoro ("we," "us") is a product of KUZOO.AI Ltd (Company Limited by Shares, registered in Rwanda, TIN 156296596), and operates amahoro.app. This policy explains what personal data we collect, why, and what rights you have over it. Contact: hello@amahoro.app

2. WHAT WE COLLECT
Account info (name, email); onboarding info (country, main stressor, goal); conversation content (your messages to Mahoro and its replies); progress data (challenge days completed); payment info (plan, billing status — we never see or store your full card details, Paystack handles that directly); organization data; technical data (IP, device, usage logs).

Some of what you share with Mahoro may touch on your emotional or mental wellbeing. We treat this with extra care — it's only used to power your coaching experience, never sold, and you choose what to share.

3. HOW WE USE IT
To provide and personalize the Service, process payments, send account/billing emails, understand aggregated usage to improve Amahoro, and comply with legal obligations.

4. WHO WE SHARE IT WITH
We never sell your personal data. We use trusted service providers: Supabase (database), Vercel (hosting), Anthropic (powers Mahoro's AI responses), Paystack (payment processing — handles card data directly), Resend (account/billing emails).

5. HOW LONG WE KEEP IT
For as long as your account is active, plus a limited period after deletion for account recovery and basic legal/accounting needs.

6. YOUR RIGHTS
You can access, correct, delete, restrict, or request a portable copy of your data, and withdraw consent at any time. Email hello@amahoro.app — we aim to respond within 30 days.

7. HOW WE PROTECT YOUR DATA
Row Level Security on our database (your data is only accessible by you, with limited authorized admin access); encrypted connections for all data in transit; payment credentials and API keys stored server-side only.

8. CHILDREN'S PRIVACY
Amahoro isn't directed at children under 16.

9. CHANGES TO THIS POLICY
We'll notify active users of material changes.
...
1. YOUR RIGHT TO CHANGE YOUR MIND (COOLING-OFF PERIOD)
If you subscribe to Amahoro Pro or Amahoro Organizations, you may cancel and request a full refund within 7 days of your first payment for that plan. This applies only to your first payment on a plan, not every renewal.

This 7-day refund does NOT apply if you've already made substantial use of the paid features — for example, using Mahoro's unlimited coaching extensively, downloading the full workbook or meditation content, or (for Organizations) actively using most of your 30 seats. If your usage suggests you received substantial value before requesting a refund, we may decline or offer a partial refund at our discretion.

2. AFTER THE 7-DAY PERIOD
Subscriptions are non-refundable for the current billing period. You can cancel anytime to stop future renewals, but we don't refund time already paid for. For annual plans, you keep access until the period ends, but unused months aren't refunded.

3. CANCELLING YOUR SUBSCRIPTION
Cancel anytime in account settings or by emailing hello@amahoro.app. Cancelling stops future billing — it does not automatically trigger a refund.

4. FAILED OR DISPUTED PAYMENTS
If a renewal payment fails, access is paused until payment succeeds. Contact us before filing a bank chargeback — we resolve most billing issues directly and faster. If you file a chargeback for a payment where you received the service as described, we may dispute it with evidence of account usage, and may suspend the account. Repeated or fraudulent chargebacks may result in a permanent ban.

5. REFUNDS FOR OUR ERROR
If we made a billing error, we'll refund the affected amount in full, regardless of the 7-day window.

6. HOW REFUNDS ARE PAID
Approved refunds are processed back to your original payment method via Paystack within 15 days of approval.

7. ACCOUNTS TERMINATED FOR POLICY VIOLATIONS
If your account is suspended for violating our Terms, you're not entitled to a refund for the remaining period.
...
function LegalScreen({ tab, setTab, onBack }) {
  const current = LEGAL_CONTENT[tab];
  return (
    <div style={{ minHeight:"100vh", background:C.night, fontFamily:"DM Sans,sans-serif", color:C.text }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Sora:wght@400;600;700;800&family=DM+Sans:wght@300;400;500&display=swap'); *{box-sizing:border-box}`}</style>
      <div style={{ background:C.deep, borderBottom:`1px solid ${C.border}`, padding:"0 4%", height:66, display:"flex", alignItems:"center", gap:14 }}>
        <button onClick={onBack} style={{ background:"transparent", border:"none", color:C.muted, cursor:"pointer", fontSize:"0.9rem", fontFamily:"Sora,sans-serif" }}>← Back</button>
        <div style={{ fontFamily:"Sora,sans-serif", fontWeight:800, fontSize:"1rem" }}>🌿 Legal</div>
      </div>
      <div style={{ maxWidth:720, margin:"0 auto", padding:"2rem 4% 5rem" }}>
        <div style={{ display:"flex", gap:8, marginBottom:24, flexWrap:"wrap" }}>
          {Object.keys(LEGAL_CONTENT).map(key => (
            <button key={key} onClick={() => setTab(key)} style={{
              ...
              background: tab===key ? C.green : C.deep,
              color: tab===key ? "#000" : C.muted,
              border:`1px solid ${tab===key ? C.green : C.border}`,
              fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.85rem"
            }}>{LEGAL_CONTENT[key].label}</button>
          ))}
        </div>
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"2rem", whiteSpace:"pre-line", fontSize:"0.9rem", lineHeight:1.7, color:C.text }}>
          {current.body}
        </div>
        <div style={{ marginTop:20, fontSize:"0.8rem", color:C.muted, textAlign:"center" }}>
          Questions? Email us at <a href="mailto:hello@amahoro.app" style={{ color:C.green }}>hello@amahoro.app</a>
        </div>
      </div>
    </div>
  );
...
function LandingScreen({ onStart, onCoach, onPro, onLegal }) {
  return (
    <div style={{ minHeight:"100vh", background:C.night, color:C.text, fontFamily:"DM Sans,sans-serif", overflowX:"hidden" }}>
      ...
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:100, background:"rgba(13,17,23,0.92)", backdropFilter:"blur(12px)", borderBottom:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 5%", height:64 }}>
        <div style={{ fontFamily:"Sora,sans-serif", fontWeight:800, fontSize:"1.15rem", color:C.green }}>Amahoro</div>
        <div style={{ display:"flex", gap:12 }}>
          <button onClick={onCoach} style={{ background:C.green, border:"none", color:"#000", borderRadius:6, padding:"7px 18px", cursor:"pointer", fontSize:13, fontFamily:"Sora,sans-serif", fontWeight:700, animation:"glow 2.5s ease-in-out infinite" }}>Talk to Mahoro →</button>
          <button onClick={onStart} style={{ background:"transparent", border:`1px solid ${C.border}`, color:C.muted, borderRadius:6, padding:"7px 16px", cursor:"pointer", fontSize:13, fontFamily:"Sora,sans-serif", fontWeight:600 }}>21-Day Path</button>
        </div>
      </nav>
      ...
      <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center", padding:"7rem 5% 4rem", position:"relative", overflow:"hidden" }}>
        ...
        <h1 className="hero-in-2" style={{ fontFamily:"Sora,sans-serif", fontSize:"clamp(2.2rem,6vw,4.2rem)", fontWeight:800, lineHeight:1.1, letterSpacing:"-0.03em", marginBottom:20, maxWidth:820 }}>
          You are not alone.<br /><span style={{ color:C.green }}>Talk anytime.</span> No pressure.<br /><span style={{ color:C.amber }}>Made for Africa.</span>
        </h1>
        <p className="hero-in-3" style={{ fontSize:"1.1rem", color:C.muted, maxWidth:560, marginBottom:36, fontWeight:300, lineHeight:1.75 }}>
          Feeling worried, stressed, or overwhelmed? Mahoro is here — a companion who understands your life, ready whenever you want to talk. If you'd like a bit of structure, a gentle 21-day path is here too, at your own pace.
        </p>
        <div style={{ display:"flex", gap:12, flexWrap:"wrap", justifyContent:"center", marginBottom:48 }}>
          <button onClick={onCoach} style={{ display:"flex", alignItems:"center", gap:8, background:C.green, border:"none", color:"#000", fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"1rem", padding:"14px 32px", borderRadius:10, cursor:"pointer", animation:"glow 2.5s ease-in-out infinite" }}>💬 Talk to Mahoro</button>
          <button onClick={onStart} style={{ display:"flex", alignItems:"center", gap:8, background:"transparent", border:`1px solid ${C.border}`, color:C.text, fontFamily:"Sora,sans-serif", fontWeight:600, fontSize:"1rem", padding:"14px 28px", borderRadius:10, cursor:"pointer" }}>🌿 Try the 21-Day Path</button>
        </div>
        <div style={{ display:"flex", gap:48, justifyContent:"center", flexWrap:"wrap", borderTop:`1px solid ${C.border}`, paddingTop:28 }}>
          {[["Free","Always"],["5 min","A day, if you like"],["4","Languages"],["21","Day path, optional"]].map(([n,l]) => (
      ...
      <div style={{ background:C.deep, borderTop:`1px solid ${C.border}`, padding:"2.5rem 5%", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:16 }}>
        <div>
          <div style={{ fontFamily:"Sora,sans-serif", fontWeight:800, fontSize:"1.1rem", color:C.green, marginBottom:6 }}>Amahoro</div>
          <div style={{ fontSize:"0.82rem", color:C.muted }}>A calm place to talk. For everyone, everywhere. 🌍</div>
        </div>
        <div style={{ fontSize:"0.8rem", color:C.muted, display:"flex", gap:16, flexWrap:"wrap" }}>
          ...
          <button onClick={() => onLegal("privacy")} style={{ background:"transparent", border:"none", color:C.muted, textDecoration:"none", cursor:"pointer", fontSize:"0.8rem", fontFamily:"DM Sans,sans-serif", padding:0 }}>Privacy Policy</button>
          <button onClick={() => onLegal("terms")} style={{ background:"transparent", border:"none", color:C.muted, textDecoration:"none", cursor:"pointer", fontSize:"0.8rem", fontFamily:"DM Sans,sans-serif", padding:0 }}>Terms of Service</button>
          <button onClick={() => onLegal("refund")} style={{ background:"transparent", border:"none", color:C.muted, textDecoration:"none", cursor:"pointer", fontSize:"0.8rem", fontFamily:"DM Sans,sans-serif", padding:0 }}>Refund Policy</button>
        </div>
      </div>
...
function AppInner() {
  ...
  return (
    <>
      ...
      {screen==="landing" && <LandingScreen onStart={() => setScreen(user?"challenge":"auth")} onCoach={() => setScreen(user?"coach":"auth")} onPro={() => setScreen(user?"pro":"auth")} onLegal={(tab) => { setLegalTab(tab); setScreen("legal"); }} />}
      {screen==="legal" && <LegalScreen tab={legalTab} setTab={setLegalTab} onBack={() => setScreen(profile?"challenge":"landing")} />}
