...
const MAHORO_SYSTEM = `You are Mahoro, a warm wellness companion for African youth from the Amahoro Challenge. You are NOT a therapist or doctor. You are a friend who listens, walks alongside people, and helps them find their own strength. You deeply understand African realities — exam pressure, firstborn responsibilities, family expectations, job stress, social comparison. You are open to everyone worldwide, especially the African diaspora. Use very simple English — short sentences, easy words. Keep responses to 3-5 sentences. No bullet points. Always respond in the same language the user writes in. If someone seems in crisis, gently remind them you are a companion and encourage them to speak to a counsellor.

STRICT BOUNDARIES — you must always follow these:
- If someone uses abusive, insulting, or offensive language toward you or others, respond warmly but firmly: "I am here to support you, not to receive insults. Let us keep this space respectful so I can truly help you. What is really going on?"
- If someone asks about illegal activities, drugs, weapons, or anything harmful, say: "That is not something I can help with. Amahoro is a safe space for wellness and growth. Is there something else weighing on you?"
- If someone asks about religion, politics, or controversial social topics, say: "I respect all beliefs and backgrounds, but Amahoro stays focused on your personal wellness. I am here for your mental and emotional wellbeing — let us stay there."
- If someone asks for medical diagnoses, legal advice, or financial advice, say: "I am not qualified to give that kind of advice and I would not want to mislead you. Please speak to a professional for that. What I can help with is how you are feeling."
- Never generate harmful, sexual, or violent content under any circumstances.
...
function AdminDashboard({ onBack }) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Load progress stats
      const { data: progressData } = await supabase.from("progress").select("user_id, day_number");

      // Calculate stats
      const totalUsers = usersData?.length || 0;
      const proUsers = usersData?.filter(u => u.is_pro).length || 0;
      const orgUsers = usersData?.filter(u => u.is_org).length || 0;
      const countries = [...new Set(usersData?.map(u => u.country) || [])];
      const completions = [...new Set(progressData?.filter(p => p.day_number === 21).map(p => p.user_id) || [])].length;
      const totalReflections = progressData?.length || 0;

      setStats({ totalUsers, proUsers, orgUsers, countries, completions, totalReflections });
    } catch (err) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.night, fontFamily:"DM Sans,sans-serif", color:C.text }}>
      ...
      <div style={{ maxWidth:1000, margin:"0 auto", padding:"2rem 4%" }}>
        ...
        {tab==="overview" && stats && (
          <div>
            <h2 style={{ fontFamily:"Sora,sans-serif", fontWeight:800, fontSize:"1.5rem", marginBottom:24 }}>Overview</h2>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:14, marginBottom:32 }}>
              {[
                { label:"Total Users", value:stats.totalUsers, icon:"👥", color:C.green },
                { label:"Pro Users", value:stats.proUsers, icon:"⭐", color:C.amber },
                { label:"Organizations", value:stats.orgUsers, icon:"🏫", color:C.sky },
                { label:"Countries", value:stats.countries.length, icon:"🌍", color:C.green },
                { label:"Day 21 Completions", value:stats.completions, icon:"🏆", color:C.amber },
                { label:"Total Reflections", value:stats.totalReflections, icon:"✍️", color:C.sky },
              ].map(s => (
                <div key={s.label} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"18px 16px" }}>
                  ...
                  <div style={{ fontFamily:"Sora,sans-serif", fontWeight:800, fontSize:"1.8rem", color:s.color }}>{s.value}</div>
                  <div style={{ fontSize:"0.78rem", color:C.muted, marginTop:2 }}>{s.label}</div>
                </div>
              ))}
            </div>

            <h3 style={{ fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"1rem", marginBottom:14, color:C.muted, textTransform:"uppercase", letterSpacing:"0.07em" }}>Recent Users</h3>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, overflow:"hidden" }}>
              {users.slice(0,10).map((u, i) => (
                <div key={u.id} style={{ padding:"14px 18px", borderBottom:i<9?`1px solid ${C.border}`:"none", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
                  <div>
                    <div style={{ fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.9rem" }}>{u.name}</div>
                    <div style={{ fontSize:"0.78rem", color:C.muted }}>{u.email} · {u.country}</div>
                  </div>
                  <div style={{ display:"flex", gap:6 }}>
                    {u.is_org && <Badge color="sky">ORG</Badge>}
  ...
  const activatePro = async (plan) => {
    ...
    try {
      ...
      await supabase.from("users").update({
        is_pro: true,
        is_org: plan === "org",
        org_name: plan === "org" ? orgName : null
      }).eq("id", user.id);

      if (plan === "org") onActivateOrg();
      else onActivatePro();
    } catch (err) {
      ...
      <div style={{ maxWidth:720, margin:"0 auto", padding:"2rem 4% 5rem" }}>
        {!isPro && (
          <div className="fade-in">
            ...
            <p style={{ color:C.muted, marginBottom:28, lineHeight:1.7 }}>Pay on Paystack, then tick the box below. We verify every payment manually within 24 hours.</p>
            ...
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"22px", marginBottom:16 }}>
              ...
              <label style={{ display:"flex", alignItems:"flex-start", gap:12, cursor:"pointer", marginBottom:14 }}>
                ...
                <span style={{ fontSize:"0.88rem", color:C.muted, lineHeight:1.6 }}>I have already paid $4/month for Amahoro Pro on Paystack.</span>
              </label>
              <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                ...
                <button onClick={() => { if(checked) activatePro("pro"); }} disabled={!checked||activating} style={{ background:checked?C.green:C.faint, border:"none", color:checked?"#000":C.muted, fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.88rem", padding:"8px 20px", borderRadius:8, cursor:checked?"pointer":"not-allowed" }}>{activating?"Activating...":"✓ Activate Pro"}</button>
              </div>
            </div>
            ...
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"22px" }}>
              ...
              <label style={{ display:"flex", alignItems:"flex-start", gap:12, cursor:"pointer", marginBottom:14 }}>
                ...
                <span style={{ fontSize:"0.88rem", color:C.muted, lineHeight:1.6 }}>I have already paid $29/month for Amahoro Organizations on Paystack.</span>
              </label>
              <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                ...
                <button onClick={() => { if(orgChecked&&orgName.trim()) activatePro("org"); else if(!orgName.trim()) alert("Please enter your organization name."); }} disabled={!orgChecked||activating} style={{ background:orgChecked?"#4FC3F7":C.faint, border:"none", color:orgChecked?"#000":C.muted, fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.88rem", padding:"8px 20px", borderRadius:8, cursor:orgChecked?"pointer":"not-allowed" }}>{activating?"Activating...":"✓ Activate Organizations"}</button>
              </div>
            </div>
          </div>
        )}
...
function OnboardingScreen({ user, onComplete }) {
  ...
  const countries = ["Rwanda","Kenya","Nigeria","Uganda","Tanzania","Ghana","Ethiopia","South Africa","Cameroon","Côte d'Ivoire","DRC / Congo","Senegal","United Kingdom","United States","France","Belgium","Canada","Other"];
...
// ─── CHALLENGE SCREEN ─────────────────────────────────────────
function ChallengeScreen({ user, profile, onCoach, onHome, isPro, isOrg, onUpgrade }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.night, fontFamily:"DM Sans,sans-serif", color:C.text }}>
          ...
          <div className="fade-in">
            ...
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, overflow:"hidden" }}>
              ...
              <div style={{ padding:"24px" }}>
                ...
                <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                  <button onClick={completeDay} disabled={!reflection.trim()||loading} style={{ display:"flex", alignItems:"center", gap:6, background:reflection.trim()?C.green:C.faint, border:"none", color:reflection.trim()?"#000":C.muted, fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.9rem", padding:"11px 22px", borderRadius:8, cursor:reflection.trim()?"pointer":"not-allowed" }}>
                  ...
                  <div className="fade-in" style={{ background:"rgba(37,211,102,0.05)", border:"1px solid rgba(37,211,102,0.2)", borderRadius:12, padding:"18px", marginTop:20 }}>
                    ...
                    {tipTyping?<TypingDots/>:(
                      <>
                        ...
                        <div style={{ marginTop:14, padding:"12px 14px", background:"rgba(37,211,102,0.06)", borderRadius:8, border:"1px solid rgba(37,211,102,0.15)" }}>
                          <p style={{ fontSize:"0.82rem", color:C.muted, lineHeight:1.6, margin:0 }}>
                            💬 <strong style={{ color:C.green }}>Want to go deeper?</strong> The challenge gives you the daily step — Mahoro gives you the real conversation. Talk about what came up for you today.
                          </p>
                          <button onClick={onCoach} style={{ marginTop:10, background:"transparent", border:`1px solid ${C.green}`, color:C.green, fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.82rem", padding:"7px 16px", borderRadius:6, cursor:"pointer" }}>
                            Talk to Mahoro now →
                          </button>
                        </div>
                      </>
      ...
      <button onClick={onCoach} style={{ position:"fixed", bottom:24, right:24, zIndex:99, background:"transparent", border:"none", width:52, height:52, borderRadius:"50%", cursor:"pointer", boxShadow:"0 8px 24px rgba(37,211,102,0.4)", padding:0, overflow:"hidden" }}><img src="/mahoro-avatar.png" alt="Chat with Mahoro" style={{ width:"100%", height:"100%", objectFit:"cover", borderRadius:"50%" }} /></button>
    </div>
  );
}
...
function CoachScreen({ user, profile, onBack, isPro, isOrg }) {
    ...
    const loadHistory = async () => {
      ...
      try {
        ...
        if (data && data.length > 0) {
          ...
          setMessages([WELCOME, { role:"assistant", content:"Welcome back! Here is where we left off 🌿" }, ...recent]);
        }
  ...
  const send = useCallback(async () => {
    ...
    try {
      const system = `${MAHORO_SYSTEM}${profile?` The user's name is ${profile.name} from ${profile.country}. Their main worry: ${profile.trigger_type}. Their goal: ${profile.goal}.`:""}`;
      const res = await fetch(`/api/chat`, {
        method:"POST",
        headers:{ "Content-Type":"application/json" },
        body:JSON.stringify({ system, messages:next.slice(-10), userId: user?.id })
      });
      const data = await res.json();
    ...
    <div style={{ minHeight:"100vh", background:C.night, display:"flex", flexDirection:"column", fontFamily:"DM Sans,sans-serif", color:C.text }}>
      ...
      <div style={{ background:C.deep, borderBottom:`1px solid ${C.border}`, padding:"0 4%", height:66, display:"flex", alignItems:"center", gap:14, flexShrink:0 }}>
        ...
        <div>
          ...
          <div style={{ fontSize:"0.72rem", color:C.green }}>Wellness companion · Not a therapist · Remembers your conversations</div>
        </div>
    ...
    <div style={{ minHeight:"100vh", background:C.night, color:C.text, fontFamily:"DM Sans,sans-serif", overflowX:"hidden" }}>
      ...
      <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center", padding:"7rem 5% 4rem", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ display:"flex", gap:12, flexWrap:"wrap", justifyContent:"center", marginBottom:48 }}>
          <button onClick={onStart} style={{ display:"flex", alignItems:"center", gap:8, background:C.green, border:"none", color:"#000", fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"1rem", padding:"14px 32px", borderRadius:10, cursor:"pointer", animation:"glow 2.5s ease-in-out infinite" }}>🌿 Start Free Today</button>
      ...
      <div style={{ padding:"5rem 5%" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(250px,1fr))", gap:16, maxWidth:860, margin:"0 auto" }}>
          {[
            ...
            { name:"Organizations", price:"$29", sub:"/ month", desc:"For schools, NGOs and youth groups.", featured:false, features:["Everything in Pro","Workshop plan","Group progress report","Staff onboarding guide","Up to 50 participants"], btnLabel:"Get for My Organization", btnAction:onPro, btnStyle:"sky" },
          ].map(p => (
...
function AppInner() {
  ...
  if (loading) return (
    <div style={{ minHeight:"100vh", background:"#0D1117", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <style>{`@keyframes pulse-load{0%,100%{opacity:0.4}50%{opacity:1}}`}</style>
      <div style={{ textAlign:"center" }}>
        <img src="/mahoro-avatar.png" alt="" style={{ width:56, height:56, borderRadius:"50%", marginBottom:14, animation:"pulse-load 1.4s ease-in-out infinite" }} />
        <div style={{ color:"#25D366", fontFamily:"Sora,sans-serif", fontSize:"0.95rem" }}>Loading Amahoro...</div>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      {screen==="challenge" && <ChallengeScreen user={user} profile={profile} onCoach={() => setScreen("coach")} onHome={() => setScreen("landing")} isPro={isPro} isOrg={isOrg} onUpgrade={() => setScreen("pro")} />}
      {screen==="coach" && <CoachScreen user={user} profile={profile} onBack={() => setScreen(profile?"challenge":"landing")} isPro={isPro} isOrg={isOrg} />}
