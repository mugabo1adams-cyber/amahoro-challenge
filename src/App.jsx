          ...
          {[
            ...
            <div key={p.name} style={{ background: COLORS.card, border: `2px solid ${p.featured ? COLORS.green : COLORS.border}`, borderRadius: 16, padding: "28px 24px", position: "relative", boxShadow: p.featured ? `0 0 0 1px rgba(37,211,102,0.2), 0 20px 40px rgba(37,211,102,0.07)` : "none" }}>
              ...
              <button onClick={() => {
                ...
                if (p.name === "Amahoro Pro") { window.open("https://paystack.shop/pay/25-4wusw62", "_blank"); return; }
                if (p.name === "Organizations") { window.open("https://paystack.shop/pay/ljxed84arr", "_blank"); return; }
...
function CoachScreen({ user, onBack }) {
  const [messages, setMessages] = useState([
    { role: "assistant", content: `Habari${user?.name ? ` ${user.name}` : ""}! I'm Mahoro — Mshauri wako / Votre coach / Umujyanama wawe 🌿

I speak English, Français, Kiswahili and Kinyarwanda — just write in your language and I'll respond in the same.

What's on your mind today? / Qu'est-ce qui vous préoccupe? / Una nini moyoni? / Ni iki gikuganya umutima?` }
  ]);
