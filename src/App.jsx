...
const ANTHROPIC_KEY = "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";
      ...
      <div style={{ maxWidth:1000, margin:"0 auto", padding:"2rem 4%" }}>
        ...
        {tab==="users" && (
          <div>
            ...
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, overflow:"hidden" }}>
              {users.map((u, i) => (
                <div key={u.id} style={{ padding:"14px 18px", borderBottom:i<users.length-1?`1px solid ${C.border}`:"none", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
                  <div>
                    <div style={{ fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.9rem" }}>{u.name}</div>
...
function ProScreen({ user, profile, isPro, isOrg, onActivatePro, onActivateOrg, onBack }) {
  ...
  const generate = async (prompt) => {
    ...
    try { setOutput(await callClaude(MAHORO_SYSTEM, prompt)); }
                  ...
                  <div className="fade-in" style={{ background:"rgba(37,211,102,0.04)", border:"1px solid rgba(37,211,102,0.15)", borderRadius:10, padding:"16px" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                        <div style={{ width:28, height:28, borderRadius:"50%", background:"linear-gradient(135deg,#25D366,#4FC3F7)", display:"flex", alignItems:"center", justifyContent:"center" }}>🌿</div>
                ...
                {tipVisible && (
                  <div className="fade-in" style={{ background:"rgba(37,211,102,0.05)", border:"1px solid rgba(37,211,102,0.2)", borderRadius:12, padding:"18px", marginTop:20 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
                      <div style={{ width:34, height:34, borderRadius:"50%", background:"linear-gradient(135deg,#25D366,#4FC3F7)", display:"flex", alignItems:"center", justifyContent:"center" }}>🌿</div>
                    ...
                    {tipTyping?<TypingDots/>:<p style={{ fontSize:"0.9rem", lineHeight:1.7 }}>{aiTip}</p>}
                  </div>
                )}
...
function CoachScreen({ user, profile, onBack, isPro, isOrg }) {
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      const data = await res.json();
      const reply = data.content?.[0]?.text || "I am here with you. Can you tell me a little more? 🌿";
    ...
    <div style={{ minHeight:"100vh", background:C.night, display:"flex", flexDirection:"column", fontFamily:"DM Sans,sans-serif", color:C.text }}>
      ...
      <div style={{ background:C.deep, borderBottom:`1px solid ${C.border}`, padding:"0 4%", height:66, display:"flex", alignItems:"center", gap:14, flexShrink:0 }}>
        ...
        <div>
          ...
          <div style={{ fontSize:"0.72rem", color:C.green }}>Your wellness companion · Not a therapist</div>
        </div>
      ...
      <div ref={chatRef} style={{ flex:1, overflowY:"auto", padding:"24px 4%", display:"flex", flexDirection:"column", gap:16, maxWidth:680, width:"100%", margin:"0 auto" }}>
        ...
        {loading&&<div className="msg-in" style={{ display:"flex", gap:10, alignItems:"flex-start" }}><div style={{ width:32, height:32, borderRadius:"50%", background:"linear-gradient(135deg,#25D366,#4FC3F7)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"0.85rem" }}>🌿</div><div style={{ padding:"12px 16px", borderRadius:"4px 12px 12px 12px", background:C.card, border:`1px solid ${C.border}` }}><TypingDots/></div></div>}
      </div>
  ...
  if (loading) return (
    <div style={{ minHeight:"100vh", background:"#0D1117", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ textAlign:"center" }}>
        ...
        <div style={{ color:"#25D366", fontFamily:"Sora,sans-serif", fontSize:"1rem" }}>Loading Amahoro...</div>
      </div>
    </div>
  );
