...
function ProScreen({ user, isPro, isOrg, onActivatePro, onActivateOrg, onBack }) {
  ...
  const API_KEY = "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";
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.night, fontFamily:"DM Sans,sans-serif", color:C.text }}>
      ...
      <div style={{ position:"fixed", top:0, left:0, right:0, zIndex:100, background:"rgba(13,17,23,0.95)", backdropFilter:"blur(12px)", borderBottom:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 4%", height:60 }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <button onClick={onHome} style={{ background:"transparent", border:"none", color:C.green, fontFamily:"Sora,sans-serif", fontWeight:800, fontSize:"1rem", cursor:"pointer" }}>🌿 Amahoro</button>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <span style={{ fontSize:"0.82rem", color:C.muted, fontFamily:"Sora,sans-serif" }}>{user.name} · {user.country}</span>
        ...
        <div style={{ display:"flex", gap:8 }}>
          {["challenge","progress","whatsapp"].map(v => (
            <button key={v} onClick={() => setView(v)} style={{ background:view===v?"rgba(37,211,102,0.1)":"transparent", border:`1px solid ${view===v?C.green:C.border}`, color:view===v?C.green:C.muted, borderRadius:6, padding:"5px 12px", cursor:"pointer", fontSize:"0.78rem", fontFamily:"Sora,sans-serif", fontWeight:600, textTransform:"capitalize" }}>{v}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ paddingTop:80, maxWidth:780, margin:"0 auto", padding:"80px 4% 4rem" }}>
            ...
            <div style={{ display:"flex", gap:6, overflowX:"auto", paddingBottom:12, marginBottom:28 }}>
              {CHALLENGES.map((c,i) => (
                <button key={i} onClick={() => { setCurrentDay(i); setReflection(""); setTipVisible(false); setAiTip(""); }}
                  style={{ flexShrink:0, width:44, height:44, borderRadius:10, border:`1px solid ${currentDay===i?C.green:completedDays.includes(i)?"rgba(37,211,102,0.4)":C.border}`, background:currentDay===i?"rgba(37,211,102,0.15)":completedDays.includes(i)?"rgba(37,211,102,0.06)":C.deep, color:currentDay===i?C.green:completedDays.includes(i)?C.green:C.muted, fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.78rem", cursor:"pointer" }}>
            ...
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, overflow:"hidden" }}>
              ...
              <div style={{ padding:"24px" }}>
                ...
                <div style={{ marginBottom:20 }}>
                  ...
                  <p style={{ fontSize:"0.92rem", lineHeight:1.7, color:C.text }}>{ch.task}</p>
                </div>
                <div style={{ background:C.deep, border:`1px solid ${C.border}`, borderRadius:10, padding:"16px", marginBottom:20 }}>
                  ...
                  <textarea value={reflection} onChange={e => setReflection(e.target.value)} placeholder={ch.placeholder} rows={5} style={{ width:"100%", background:"transparent", border:"none", color:C.text, fontFamily:"DM Sans,sans-serif", fontSize:"0.93rem", resize:"none", lineHeight:1.65, outline:"none" }} />
                </div>
                ...
                <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                  <button onClick={completeDay} disabled={!reflection.trim()||loading}
                    style={{ display:"flex", alignItems:"center", gap:6, background:reflection.trim()?C.green:C.faint, border:"none", color:reflection.trim()?"#000":C.muted, fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.9rem", padding:"11px 22px", borderRadius:8, cursor:reflection.trim()?"pointer":"not-allowed" }}>
                  ...
                  {reflection.trim() && (
                    <a href={`https://wa.me/254113445647?text=${waMessage}`} target="_blank" rel="noreferrer"
                      style={{ display:"flex", alignItems:"center", gap:6, background:"transparent", border:`1px solid ${C.green}`, color:C.green, fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.9rem", padding:"11px 20px", borderRadius:8, textDecoration:"none" }}>
                      💬 Share on WhatsApp
                    </a>
                  )}
                  {isCompleted && currentDay<20 && (
                    <button onClick={() => { setCurrentDay(d => d+1); setReflection(""); setTipVisible(false); setAiTip(""); }}
                      style={{ background:C.deep, border:`1px solid ${C.border}`, color:C.text, fontFamily:"Sora,sans-serif", fontWeight:600, fontSize:"0.9rem", padding:"11px 20px", borderRadius:8, cursor:"pointer" }}>
                      Day {currentDay+2} →
                    </button>
                  )}
                  <button onClick={onCoach} style={{ background:"transparent", border:`1px solid ${C.border}`, color:C.muted, fontFamily:"Sora,sans-serif", fontWeight:600, fontSize:"0.9rem", padding:"11px 18px", borderRadius:8, cursor:"pointer" }}>🤖 Ask Mahoro</button>
                ...
                {tipVisible && (
                  <div className="fade-in" style={{ background:"rgba(37,211,102,0.05)", border:"1px solid rgba(37,211,102,0.2)", borderRadius:12, padding:"18px", marginTop:20 }}>
                    ...
                    {tipTyping ? <TypingDots /> : <p style={{ fontSize:"0.9rem", lineHeight:1.7, color:C.text }}>{aiTip}</p>}
                  </div>
                )}
                ...
                {currentDay===20 && isCompleted && (
                  <div className="fade-in" style={{ marginTop:24, background:"rgba(37,211,102,0.06)", border:"1px solid rgba(37,211,102,0.25)", borderRadius:12, padding:"24px", textAlign:"center" }}>
                    ...
                    <p style={{ color:C.muted, fontSize:"0.9rem", lineHeight:1.7, marginBottom:20 }}>You showed up for 21 days. You are not the same person who started Day 1. Amahoro — peace be with you.</p>
                    <a href={`https://wa.me/254113445647?text=I%20just%20completed%20the%20Amahoro%2021-Day%20Challenge%21%20%F0%9F%8F%86%20My%20name%20is%20${user.name}%20from%20${user.country}.`} target="_blank" rel="noreferrer"
                      style={{ display:"inline-flex", alignItems:"center", gap:6, background:C.green, border:"none", color:"#000", fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.9rem", padding:"11px 20px", borderRadius:8, textDecoration:"none" }}>
                      🏆 Share Your Win
                    </a>
                  </div>
                )}
        ...
        {view==="progress" && (
          <div className="fade-in">
            ...
            <p style={{ color:C.muted, marginBottom:28 }}>{completedDays.length} of 21 days done · Keep going, {user.name}.</p>
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:16, padding:"24px", marginBottom:20 }}>
              ...
              <div style={{ height:8, background:C.deep, borderRadius:4 }}>
                <div style={{ height:"100%", width:`${(completedDays.length/21)*100}%`, background:`linear-gradient(90deg,${C.green},#4FC3F7)`, borderRadius:4, transition:"width 0.6s ease" }} />
              </div>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))", gap:10 }}>
              {CHALLENGES.map((c,i) => (
                <div key={i} onClick={() => { setCurrentDay(i); setView("challenge"); }} style={{ background:C.card, border:`1px solid ${completedDays.includes(i)?"rgba(37,211,102,0.4)":C.border}`, borderRadius:12, padding:"16px 14px", cursor:"pointer", textAlign:"center" }}>
                  ...
                  <div style={{ fontSize:"0.78rem", color:C.muted, marginTop:2 }}>{c.theme}</div>
                </div>
              ))}
            </div>
            <div style={{ marginTop:28, display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:14 }}>
              {[
                { icon:"📅", label:"Days Done", value:completedDays.length },
                { icon:"🧠", label:"Worry Score", value:`↓ ${completedDays.length*3}%` },
                { icon:"🔥", label:"Current Streak", value:`${completedDays.length} days` },
                { icon:"🏆", label:"Days Left", value:21-completedDays.length },
              ].map(s => (
                <div key={s.label} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:"20px", display:"flex", gap:14, alignItems:"center" }}>
                  <div style={{ fontSize:"1.8rem" }}>{s.icon}</div>
                  <div><div style={{ fontFamily:"Sora,sans-serif", fontWeight:800, fontSize:"1.5rem", color:C.green }}>{s.value}</div><div style={{ fontSize:"0.8rem", color:C.muted }}>{s.label}</div></div>
                </div>
              ))}
            </div>
          </div>
        ...
        {view==="whatsapp" && (
          <div className="fade-in">
            <h2 style={{ fontFamily:"Sora,sans-serif", fontWeight:800, fontSize:"1.7rem", letterSpacing:"-0.025em", marginBottom:8 }}>WhatsApp</h2>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:16 }}>
              {[
                { icon:"🌅", title:"Daily Challenge", desc:"Get your daily task every morning as a WhatsApp message.", link:`https://wa.me/254113445647?text=START%20${encodeURIComponent(user.name)}`, label:"Subscribe to Daily Messages" },
                { icon:"💬", title:"Share Your Answer", desc:"Send your answer directly to Mahoro for personal feedback.", link:`https://wa.me/254113445647?text=Day%20${ch.day}%20answer:%20`, label:"Send Answer to Mahoro" },
                ...
                { icon:"📊", title:"Weekly Update", desc:"Ask Mahoro for your weekly progress and next steps.", link:`https://wa.me/254113445647?text=REPORT%20${encodeURIComponent(user.name)}`, label:"Get Weekly Update" },
              ].map(item => (
                <div key={item.title} style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:14, padding:"22px" }}>
                  ...
                  <p style={{ fontSize:"0.85rem", color:C.muted, lineHeight:1.65, marginBottom:16 }}>{item.desc}</p>
                  <a href={item.link} target="_blank" rel="noreferrer" style={{ display:"flex", alignItems:"center", gap:6, background:C.green, border:"none", color:"#000", fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.85rem", padding:"10px 16px", borderRadius:8, textDecoration:"none", width:"fit-content" }}>
                    💬 {item.label}
                  </a>
                </div>
              ))}
      ...
      <button onClick={onCoach} style={{ position:"fixed", bottom:24, right:24, zIndex:99, background:C.green, border:"none", color:"#000", width:52, height:52, borderRadius:"50%", fontSize:"1.3rem", cursor:"pointer", boxShadow:"0 8px 24px rgba(37,211,102,0.4)", display:"flex", alignItems:"center", justifyContent:"center" }}>🌿</button>
    </div>
  );
}
...
function CoachScreen({ user, onBack, isPro, isOrg }) {
  const [messages, setMessages] = useState([
    { role:"assistant", content:`Hello${user?.name?` ${user.name}`:""}! I'm Mahoro. 🌿\n\nI am not a therapist or a doctor. I am a companion — someone who walks with you, listens without judging, and helps you find your own strength. Think of me as a friend who happens to know a few useful things about managing worry and stress.\n\nI respond in English, Français, Kiswahili and Kinyarwanda. A note on Kinyarwanda and Swahili — I am still learning to speak them the way people really do. I understand the heart of what you share, but I may miss some local expressions. I am always improving.\n\nAmahoro is for everyone, everywhere — whether you are in Kigali, London, New York or anywhere in the world. Our focus is African realities, but our door is open to all.\n\nWhat is on your mind today? I am here. 🌿` }
  ]);
  ...
  const send = useCallback(async () => {
    ...
    try {
      const system = `You are Mahoro, a warm wellness companion for the Amahoro Challenge. You are NOT a therapist, doctor or mental health professional. You are a friend who listens, walks alongside people, and helps them find their own strength. You deeply understand African realities — exam pressure, firstborn responsibilities, family expectations, job stress, social comparison, the weight of being "the successful one." You are also open to everyone worldwide, especially the African diaspora. Use very simple English — short sentences, easy words, like talking to a friend. Keep responses to 3-5 sentences. No bullet points. Always respond in the same language the user writes in. If they write in French, reply in French. If Swahili, reply in Swahili. If Kinyarwanda, reply in Kinyarwanda. If you are ever concerned someone is in crisis, gently remind them that you are a companion not a professional, and encourage them to speak to a counsellor or trusted person.${user?` The user's name is ${user.name} from ${user.country}. Their main worry: ${user.trigger}. Their goal: ${user.goal}.`:""}`;
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method:"POST",
        headers:{ "Content-Type":"application/json", "x-api-key":API_KEY, "anthropic-version":"2023-06-01", "anthropic-dangerous-direct-browser-access":"true" },
        body:JSON.stringify({ model:"claude-sonnet-4-6", max_tokens:1000, system, messages:next.slice(-10) })
      });
      const data = await res.json();
      setMessages(m => [...m, { role:"assistant", content:data.content?.[0]?.text||"I am here with you. Can you tell me a little more about what you are feeling? 🌿" }]);
    } catch (err) {
      console.error("Mahoro API error:", err);
      const key = import.meta.env.VITE_ANTHROPIC_API_KEY;
      const errMsg = !key 
        ? "I cannot connect right now — the API key is missing. Please contact hello@amahoro.app 🌿"
        : "I had a connection problem. Please check your internet and try again. 🌿";
      setMessages(m => [...m, { role:"assistant", content: errMsg }]);
    }
    setLoading(false);
  }, [input, messages, loading, user, API_KEY]);
    ...
    <div style={{ minHeight:"100vh", background:C.night, display:"flex", flexDirection:"column", fontFamily:"DM Sans,sans-serif", color:C.text }}>
      ...
      <div style={{ background:C.deep, borderBottom:`1px solid ${C.border}`, padding:"0 4%", height:66, display:"flex", alignItems:"center", gap:14, flexShrink:0 }}>
        ...
        <div>
          ...
          <div style={{ fontSize:"0.75rem", color:C.green }}>Your wellness companion · Not a therapist</div>
        </div>
        <div style={{ marginLeft:"auto" }}>
          <a href="https://wa.me/254113445647?text=Hi%20Mahoro!" target="_blank" rel="noreferrer" style={{ display:"flex", alignItems:"center", gap:6, background:C.green, border:"none", color:"#000", fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.8rem", padding:"7px 14px", borderRadius:8, textDecoration:"none" }}>
      ...
      <div ref={chatRef} style={{ flex:1, overflowY:"auto", padding:"24px 4%", display:"flex", flexDirection:"column", gap:16, maxWidth:680, width:"100%", margin:"0 auto" }}>
        {messages.map((m,i) => (
          <div key={i} className="msg-in" style={{ display:"flex", gap:10, alignItems:"flex-start", flexDirection:m.role==="user"?"row-reverse":"row" }}>
            {m.role==="assistant" && <div style={{ width:32, height:32, borderRadius:"50%", background:"linear-gradient(135deg,#25D366,#4FC3F7)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"0.85rem", flexShrink:0 }}>🌿</div>}
            <div style={{ maxWidth:"78%", padding:"12px 16px", borderRadius:m.role==="user"?"12px 4px 12px 12px":"4px 12px 12px 12px", background:m.role==="user"?"rgba(37,211,102,0.12)":C.card, border:m.role==="user"?"1px solid rgba(37,211,102,0.2)":`1px solid ${C.border}`, fontSize:"0.92rem", lineHeight:1.7, color:C.text, whiteSpace:"pre-wrap" }}>
              {m.content}
            </div>
          </div>
        ))}
        {loading && (
          <div className="msg-in" style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
            ...
            <div style={{ padding:"12px 16px", borderRadius:"4px 12px 12px 12px", background:C.card, border:`1px solid ${C.border}` }}><TypingDots /></div>
          </div>
        )}
      </div>
      ...
      {messages.length===1 && (
        <div style={{ padding:"0 4%", maxWidth:680, width:"100%", margin:"0 auto", display:"flex", gap:8, flexWrap:"wrap", marginBottom:12 }}>
          {["I am overwhelmed by family expectations","I cannot stop thinking too much at night","I am scared of failing my exams","I feel worried but I do not know why"].map(p => (
...
export default function App() {
  ...
  return (
    <>
      ...
      {screen==="onboard" && <OnboardingScreen onComplete={(data) => { setUser(data); setScreen("challenge"); }} />}
      {screen==="challenge" && <ChallengeScreen user={user||{name:"Friend",country:"the world",trigger:"general stress",goal:"feel calmer"}} onCoach={() => setScreen("coach")} onHome={() => setScreen("landing")} isPro={isPro} isOrg={isOrg} onUpgrade={() => setScreen("pro")} />}
      {screen==="coach" && <CoachScreen user={user} onBack={() => setScreen(user?"challenge":"landing")} isPro={isPro} isOrg={isOrg} />}
      {screen==="pro" && <ProScreen user={user} isPro={isPro} isOrg={isOrg} onActivatePro={() => { setIsPro(true); setScreen("challenge"); }} onActivateOrg={() => { setIsPro(true); setIsOrg(true); setScreen("challenge"); }} onBack={() => setScreen(user?"challenge":"landing")} />}
    </>
  );
}
