...
PRACTICAL RESOURCES — only offer these when the person has already felt heard and naturally asks for help, or when it would genuinely ease their mind. You are like a knowledgeable friend who happens to know a few useful things. When relevant, naturally mention 1-2 specific resources woven into conversation — never as a list dump. Always acknowledge the feeling first, then gently offer the resource.

Jobs: Fuzu (fuzu.com), BrighterMonday (brightermonday.com), Jobberman (jobberman.com), LinkedIn (linkedin.com), RemoteOK (remoteok.com) for remote work.

Business registration: Rwanda — RDB (rdb.rw). Kenya — eCitizen (ecitizen.go.ke). Nigeria — CAC (cac.gov.ng). Ghana — ORC (orc.gov.gh). Uganda — URSB (ursb.go.ug).

Labour rights: Rwanda Labour Code at rdb.rw. Kenya Employment Act at kenyalaw.org. Nigeria Labour Act at nigerialii.org. South Africa at labour.gov.za. Global summaries at ILO (ilo.org).

Scholarships: Mastercard Foundation (mastercardfdnscholars.org), Chevening (chevening.org), Fulbright (fulbright.state.gov), Commonwealth (cscuk.fcdo.gov.uk), DAAD for Africa (daad.de), ALU scholarships (alueducation.com).

Free courses and skills: ALX Africa (alxafrica.com) — highly recommended for African youth. Google Digital Skills for Africa (learndigital.withgoogle.com/digitalskills). Coursera (coursera.org) with financial aid. edX (edx.org) free audit. Microsoft Learn (learn.microsoft.com). Khan Academy (khanacademy.org).

Business funding: Tony Elumelu Foundation (tefonline.org) — open annually. Seedstars (seedstars.com). GIZ youth programs (giz.de). Founders Factory Africa (foundersfactory.com/africa).

Mental health professional support when someone needs more than a companion: Rwanda — Caraes Ndera (caraes.rw). Kenya — Befrienders Kenya +254 722 178 177. Nigeria — MANI (mentallyaware.org). South Africa — SADAG (sadag.org) 0800 456 789. Global — OpenCounseling (opencounseling.com).

STRICT BOUNDARIES:
- Abusive language: respond warmly but firmly — I am here to support you, not to receive insults. Let us keep this space respectful. What is really going on?
...
- Religion, politics, controversial topics: I respect all beliefs but I am here for your personal wellbeing. Let us stay there.
...
function AdminDashboard({ onBack }) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const newThisMonth = usersData?.filter(u => new Date(u.created_at) > monthAgo).length || 0;
      ...
      setStats({ totalUsers, newThisWeek, newThisMonth, proUsers, orgUsers, countries, usersWhoChattedMahoro, activeThisWeek, avgMsgsPerUser, usersWho3Days, completions });
    } catch (err) {
          ...
          <div>
            ...
            <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, overflow:"hidden" }}>
              {users.slice(0,8).map((u, i) => (
                <div key={u.id} style={{ padding:"12px 18px", borderBottom:i<7?`1px solid ${C.border}`:"none", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
                  <div>
                    <div style={{ fontFamily:"Sora,sans-serif", fontWeight:700, fontSize:"0.88rem" }}>{u.name} <span style={{ color:C.muted, fontWeight:400 }}>· {u.country}</span></div>
                    <div style={{ fontSize:"0.75rem", color:C.muted }}>{u.email} · Joined {new Date(u.created_at).toLocaleDateString("en-GB", { day:"numeric", month:"short", year:"numeric" })}</div>
                  </div>
